    ...
    try:
        ...
        for original_filename in os.listdir(utils.EXCEL_FOLDER):
            if original_filename.lower().endswith(('.xlsx', '.xls')):
                cleaned_original_sheet_name = original_filename.replace(".xlsx", "").replace(".xls", "")
                cleaned_original_sheet_name = "".join(c for c in cleaned_original_sheet_name if c.isalnum() or c in [' ', '_', '-'])
                cleaned_original_sheet_name = cleaned_original_sheet_name[:31]

                # Compare with sheet names from the imported Excel
                for imported_sheet_name, df_to_save in imported_dfs.items():
                    cleaned_imported_sheet_name = imported_sheet_name.replace(".xlsx", "").replace(".xls", "")
                    cleaned_imported_sheet_name = "".join(c for c in cleaned_imported_sheet_name if c.isalnum() or c in [' ', '_', '-'])
                    cleaned_imported_sheet_name = cleaned_imported_sheet_name[:31]

                    if cleaned_original_sheet_name == cleaned_imported_sheet_name:
                        original_file_path = os.path.join(utils.EXCEL_FOLDER, original_filename)
                        df_to_save.to_excel(original_file_path, index=False)
                        updated_count += 1
                        break # Move to next original file once a match is found

        if updated_count > 0:
            return True, f"{updated_count} fichier(s) Excel mis à jour avec succès."
        return False, "Aucun fichier Excel correspondant n'a été trouvé ou mis à jour."
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const PERMISSION_BLUEPRINTS = ['rdv', 'routes', 'facturation', 'biologie', 'radiologie', 'pharmacie', 'comptabilite', 'statistique', 'gestion_patient'];
      ...
      function generateCheckboxes(containerId, currentAllowedPages = [], isCreateForm = false, userRole = null) {
          ...
          PERMISSION_BLUEPRINTS.forEach(bp_name => {
              ...
              if (isCreateForm) {
                  ...
                  const defaultCreateChecked = ['rdv', 'routes', 'facturation', 'biologie', 'radiologie', 'pharmacie', 'comptabilite', 'statistique', 'gestion_patient'];
