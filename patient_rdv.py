...
# ------------------------------------------------------------------
def set_patient_rdv_dirs(admin_email_prefix: str):
    """
    Définit les chemins de répertoires dynamiques pour le blueprint RDV patient
    en fonction du préfixe de l'administrateur (qui est supposé déjà sanitizé).
    Ceci est crucial pour que les données soient stockées dans le bon dossier de l'admin.
    """
    global EXCEL_DIR, EXCEL_FILE, BASE_PATIENT_FILE, DISABLED_PERIODS_FILE

    # Assurez-vous que utils.DYNAMIC_BASE_DIR est défini.
    # Il est censé l'être via le app.before_request dans app.py,
    # qui appelle utils.set_dynamic_base_dir avec l'email de l'administrateur.
    if utils.DYNAMIC_BASE_DIR is None:
        print("ERREUR: utils.DYNAMIC_BASE_DIR n'est pas défini. Impossible d'initialiser les chemins RDV patient.")
        # Fallback pour éviter le crash, mais cela indique un problème dans l'ordre d'appel
        # ou la logique de session si cette erreur se produit.
        utils.set_dynamic_base_dir(f"{admin_email_prefix}@example.com") # Tenter de définir si ce n'est pas déjà fait

    # UTILISER DIRECTEMENT utils.DYNAMIC_BASE_DIR
    # base_dir_for_patient_view doit être la racine des données de l'administrateur (MEDICALINK_DATA/<admin_folder>)
    # C'est ce que utils.DYNAMIC_BASE_DIR représente.
    base_dir_for_patient_view = Path(utils.DYNAMIC_BASE_DIR)
    # Les dossiers Excel et PDF sont des sous-dossiers de DYNAMIC_BASE_DIR
    excel_folder_for_patient_view = base_dir_for_patient_view / "Excel"
    config_folder_for_patient_view = base_dir_for_patient_view / "Config"


    EXCEL_DIR = excel_folder_for_patient_view
    # Crée les répertoires si ils n'existent pas
    os.makedirs(EXCEL_DIR, exist_ok=True)
    os.makedirs(config_folder_for_patient_view, exist_ok=True) # Assurer l'existence du dossier Config

    EXCEL_FILE          = EXCEL_DIR / "DonneesRDV.xlsx"
    BASE_PATIENT_FILE   = EXCEL_DIR / "info_Base_patient.xlsx"
    DISABLED_PERIODS_FILE = config_folder_for_patient_view / "disabled_periods.json" # Chemin du nouveau fichier
    print(f"DEBUG: Chemins RDV patient définis. EXCEL_DIR: {EXCEL_DIR}, DISABLED_PERIODS_FILE: {DISABLED_PERIODS_FILE}")
...
def patient_rdv_home(admin_prefix):
    """
    Route principale pour la prise de rendez-vous par les patients.
    L'admin_prefix est automatiquement extrait de l'URL par Flask.
    """
    # Définir les répertoires de données spécifiques à cet administrateur
    # Cette fonction est également appelée dans app.before_request, mais le garder ici
    # assure que les chemins sont toujours corrects même en cas d'accès direct ou de test de la route.

    # Pour que utils.load_config() charge la bonne configuration pour cet admin,
    # nous devons temporairement définir le contexte de l'administrateur dans utils.
    original_admin_email_in_utils = utils.ADMIN_EMAIL # Sauvegarder l'état actuel de utils.ADMIN_EMAIL
    try:
        # Reconstruire l'e-mail complet attendu par utils.set_dynamic_base_dir.
        # Cela suppose que admin_prefix est la partie de l'e-mail avant '@',
        # et que le domaine est '.gmail.com' (basé sur les logs fournis précédemment).
        # C'est une hypothèse critique pour que le chemin soit correct.
        full_email_for_utils = f"{admin_prefix}@gmail.com" # Exemple: "saja@gmail.com" si admin_prefix est "saja"

        # Appeler set_dynamic_base_dir pour définir les chemins utils dans le contexte de cet admin_prefix
        utils.set_dynamic_base_dir(full_email_for_utils)
        config = utils.load_config() # Charger la config de l'admin spécifié
    finally:
        # Restaurer l'état original de utils.ADMIN_EMAIL après avoir chargé la config
        if original_admin_email_in_utils:
            utils.ADMIN_EMAIL = original_admin_email_in_utils
            # Rappeler set_dynamic_base_dir pour restaurer le contexte des utilitaires
            utils.set_dynamic_base_dir(original_admin_email_in_utils)
        else:
            # Si utils.ADMIN_EMAIL était None avant, le remettre à None ou à un état par défaut
            utils.ADMIN_EMAIL = None


    set_patient_rdv_dirs(admin_prefix) # Cette fonction utilisera maintenant utils.DYNAMIC_BASE_DIR
...
def get_reserved_slots_patient(admin_prefix):
    """
    Endpoint AJAX pour récupérer les créneaux horaires réservés et désactivés pour une date donnée.
    L'admin_prefix est automatiquement extrait de l'URL par Flask.
    """
    # Définir les répertoires de données spécifiques à cet administrateur
    # Temporairement définir le contexte de l'administrateur dans utils pour charger la bonne configuration
    original_admin_email_in_utils = utils.ADMIN_EMAIL
    try:
        # Reconstruire l'e-mail complet attendu par utils.set_dynamic_base_dir.
        full_email_for_utils = f"{admin_prefix}@gmail.com"

        utils.set_dynamic_base_dir(full_email_for_utils)
        config = utils.load_config() # NOUVEAU : Charger la config ici aussi
        set_patient_rdv_dirs(admin_prefix) # Passer l'admin_prefix ici pour bien définir les chemins
    finally:
        if original_admin_email_in_utils:
            utils.ADMIN_EMAIL = original_admin_email_in_utils
            utils.set_dynamic_base_dir(original_admin_email_in_utils)
...
def manage_disabled_periods(admin_prefix):
    ...
    if session.get('role') != 'admin':
        ...
        return redirect(url_for('login.login')) # Rediriger vers la page de connexion

    # Assurez-vous que les chemins sont définis pour cet administrateur
    full_email_for_utils = f"{admin_prefix}@gmail.com"
    utils.set_dynamic_base_dir(full_email_for_utils)
    config = utils.load_config() # NOUVEAU : Charger la config ici aussi
    set_patient_rdv_dirs(admin_prefix) # Important pour définir DISABLED_PERIODS_FILE
